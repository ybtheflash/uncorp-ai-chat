...
export function Sidebar() {
  ...
  const [colorScheme, setColorScheme] = useState(() => {
    if (typeof window !== "undefined") {
      return Cookies.get("colorScheme") || "lilac";
    }
    return "lilac";
  }); // default
  ...
  // Save user theme preferences on change
  useEffect(() => {
    if (user) {
      setDoc(
  ...
  useEffect(() => {
    // When theme changes, also update the cookie (for SSR and reload)
    if (typeof window !== "undefined") {
      Cookies.set("theme", theme || "light", { expires: 365 });
    }
  }, [theme]);
          ...
          <div className="w-full max-w-md bg-background dark:bg-zinc-900 rounded-2xl shadow-lg p-6 m-4 border max-h-[80vh] overflow-y-auto">
            ...
            {archivedChats.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {renderChatList("Archived", archivedChats, true)}
              </div>
            )}
          </div>
        </div>
